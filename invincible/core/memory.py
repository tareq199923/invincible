...
class MemoryStore:
    ...
    async def record(self, session_id: str, messages_list: list) -> int:
        added = 0
    ...
    async def facts_for(self, session_id: str,
        ...
        async with self.engine.connect() as conn:
            rows = (await conn.execute(
                ...
                .where(facts.c.user_id == SENTINEL_USER_ID,
...
async def record_turns(memory, session_id: str, messages_list: list) -> None:
    """Extract and store facts from a request's new turns (best-effort)."""
    if memory is None or not memory_enabled():
        return
    await memory.record(session_id, messages_list)
