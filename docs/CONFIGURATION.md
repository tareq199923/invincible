...
| `INVINCIBLE_OWNER_SECRET` | account sessions | Signs account browser sessions (dashboard login, OAuth consent). **Not** sent on `/mcp` — requests use short-lived OAuth Bearer tokens. **If unset, browser sessions fail closed (no login, no consent).** Legacy alias `MCP_SHARED_SECRET` is read as a fallback. |
...
| `INVINCIBLE_MEMORY` | per-request | **On by default** (Phase 4): scoped memory writes at persist time and lexical retrieval on read. Set `0`/`false`/`off` to silence both. The legacy `facts` table is no longer written or read by request-serving service code; the legacy importer remains its only production writer. |
...
- Schema: created by `invincible db upgrade` (packaged Alembic migrations;
  ...
  reads or writes `facts`; the legacy importer is its only production writer.
  Message payloads are JSONB; turn boundaries mirror the router's
