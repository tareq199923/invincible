...
|---|---|---|
| `INVINCIBLE_DB_URL` | startup (**required since Phase 16**) | PostgreSQL DSN for **all** persistent state — conversations, OAuth grants, facts, task state, staged approvals, BYOK credentials. On a remote deployment point it at a reachable managed PostgreSQL, e.g. `postgresql+asyncpg://invincible_app:***@your-db-host:5432/invincible` (Neon, RDS, Azure Database for PostgreSQL, a container, your own cluster). Laptop shortcuts: `invincible dev-db` (local, dev-only) or the bundled `docker compose up` pair — see the compose role model under Database. The DSN embeds credentials — treat it like a secret; `doctor` always prints it masked. The required production permission model (two roles) is in [SECURITY.md](SECURITY.md) §8 and [DEPLOYMENT.md](DEPLOYMENT.md). |
