...
|---|---|---|
| `INVINCIBLE_DB_URL` | startup (**required since Phase 16**) | PostgreSQL DSN for **all** persistent state — conversations, OAuth grants, legacy `facts` history, task state, staged approvals, BYOK credentials. On a remote deployment point it at a reachable managed PostgreSQL, e.g. `postgresql+asyncpg://invincible_app:***@your-db-host:5432/invincible` (Neon, RDS, Azure Database for PostgreSQL, a container, your own cluster). Laptop shortcuts: `invincible dev-db` (local, dev-only) or the bundled `docker compose up` pair — see the compose role model under Database. The DSN embeds credentials — treat it like a secret; `doctor` always prints it masked. The required production permission model (two roles) is in [SECURITY.md](SECURITY.md) §8 and [DEPLOYMENT.md](DEPLOYMENT.md). |
...
| `INVINCIBLE_MEMORY` | per-request | **On by default** (Phase 4): scoped memory writes at persist time and lexical retrieval on read. Set `0`/`false`/`off` to silence both. The legacy `facts` table is retained for historical records but is no longer written or read by request-serving service code. |
