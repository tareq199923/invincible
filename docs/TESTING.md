...
- `pytest.ini` also defaults to `-m "not slow"`: the packaging smoke test
  (`test_wheel_contents.py` — builds a wheel, creates a scratch venv,
  pip-installs into it) is marked `slow` and runs only on demand via
  `pytest -m slow` or in the release workflow. An explicit `-m` on the
  command line overrides the default.
...
- **`client`** — an `httpx.AsyncClient(transport=httpx.ASGITransport(app=app),
  base_url="http://test")`; sets `INVINCIBLE_OWNER_SECRET=test-owner-secret`
...
| `test_cli_db.py` | `dev-db` provisioning and URL normalization; `db upgrade` requires a DSN, runs the full Alembic head, and is idempotent; the `db` group registers `upgrade` alone and the retired `import` command is **unreachable**, not just hidden (exit 2, "No such command"). |
...
| `test_dual_realm.py` | *(deleted in Phase 2 — the gateway-key realm is gone; see `test_chat_byok.py` for per-user auth acceptance.)* |
...
| `test_isolation.py` | **Phase 2 acceptance**: graph hides another user's session (known:false, empty projection) with byte-identical negative shapes under enumeration probes — and the dormant operator role grants nothing on user-scoped surfaces; MCP task states/checkpoints are invisible across principals and independent chains coexist under one shared string; runs carry the owner's surrogate session; memories extracted from A's chat (via `extract_facts` into `memories` — the legacy `facts` table was dropped by revision `0013`) never reach B's outgoing context; staged `execute_bash` cannot be approved by a different subject (unknown-token semantics, action left intact). |
...
| `test_memory.py` | Deterministic extractor patterns (name/preference/decision/task relations, user messages only — assistant/tool text never mints rows), batch dedup, target caps/single-line; history turn-cap retention on/off; end-to-end: an explicitly saved fact is retrieved into a later request's provider payload only when the question's terms match, injected `[Relevant memory]` blocks are never persisted, and `INVINCIBLE_MEMORY=0` means no writes and no injections. |
| `test_dashboard.py` | **Phase 5A overview**: `/dashboard` requires a browser session; vendored `/static/htmx.min.js` is served and linked from the base template for every page; empty state renders; count cards seed correctly, stay per-user isolated (another owner's rows invisible), and recent sessions cap at 10. |
| `test_dashboard_sessions.py` | **Phase 5B views**: pages require a session; sessions listing shows owned rows with links (empty state included); detail page renders the projection pieces — runs chain, failover pairs, checkpoints, activity; **unknown and foreign surrogate ids produce byte-identical 404 bodies** (existence never leaks); tasks page lists active heads only; overview tasks card counts heads. |
| `test_dashboard_memory.py` | **Phase 5C memory management**: surfaces require a session; creation lands explicit/confidence-1.0 rows (form posts redirect back); blank/oversized content rejected; `INVINCIBLE_MEMORY=0` gates CREATION only — browse/delete stay functional so toggling off traps no data; layer/kind filters + pagination; search honors AND→OR scoped to owner; own delete audited; foreign/unknown delete identical 404s; page renders rows + search + delete buttons; overview card counts owned memories. |
| `test_dashboard_usage.py` | **Phase 5D usage**: day buckets are **identical under three different session timezones** (the UTC-pinning regression, SQL tier); surfaces require a session; totals math spans days/providers/null-token rows; `days` window filters and clamps to 1–90; per-user isolation; empty state; page renders bars + provider table; overview tokens card matches the summary. |
| `test_dashboard_settings.py` | **Phase 5E settings/password**: surfaces require a session; an `inv_*` API key NEVER authorizes the password surface; GitHub-only account sets a first password then logs in with it end-to-end; change flow rotates credentials; wrong current password rejected leaving the old credential intact; short (<8 chars) rejected on both flows; HTML form paths redirect with bounded error codes (query strings never rendered verbatim); system panel renders read-only even without a registry on `app.state`. |
| `test_session_invalidation.py` | **Session invalidation acceptance**: changing a password kills every OTHER pre-existing browser session (cookie carries a stale `session_version` → treated exactly like forged) while the acting client keeps login via a freshly minted cookie; GitHub-only first-password-set invalidates too; other users' sessions are untouched; API keys keep authenticating across the change. |
| `test_wheel_contents.py` *(slow)* | **Packaging smoke test** (`pytest -m slow`): builds the real sdist + wheel, installs the wheel into a scratch venv, and asserts from inside it — version matches `invincible/__init__.py`, `providers.yaml` resolves via `importlib.resources`, the wheel's `migration_heads()` equals the source tree's head, all dashboard templates + static assets are present, sdist carries LICENSE/README/providers.yaml.example, and `invincible --version` / `invincible harness connect --help` exit 0 with no DB and no `.env` (the README's one-command user journey). |
...
# 2. API-level (per-user inv_ key + the user's own BYOK credential):
async def test_something(client, router_setter):
    ...
    _uid, raw = await byok_user(client, "someone@example.com")
