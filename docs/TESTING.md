...
| `test_isolation.py` | **Phase 2 acceptance**: graph hides another user's session (known:false, empty projection) with byte-identical negative shapes under enumeration probes — and the dormant operator role grants nothing on user-scoped surfaces; MCP task states/checkpoints are invisible across principals and independent chains coexist under one shared string; runs carry the owner's surrogate session; facts extracted from A's chat never reach B's outgoing context; staged `execute_bash` cannot be approved by a different subject (unknown-token semantics, action left intact). |
