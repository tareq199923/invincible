...
| `test_memory.py` | Deterministic extractor patterns (name/preference/decision/task relations), batch dedup, target caps/single-line; history turn-cap retention on/off; end-to-end: an explicitly saved fact is retrieved into a later request's provider payload only when the question's terms match, injected `[Relevant memory]` blocks are never persisted, and `INVINCIBLE_MEMORY=0` means no writes and no injections. |
