...
| `test_cli_db.py` | `dev-db` provisioning and URL normalization; `db upgrade` requires a DSN, runs the full Alembic head, and is idempotent; `db --help` exposes `upgrade` but not the retired `import` command. |
