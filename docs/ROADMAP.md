...
| MCP | `POST /mcp` JSON-RPC 2.0, nineteen tools: machine-plane `read_file`, `execute_bash`, `write_file`, `confirm_action` (text-pattern denylists; single-use token approvals bound to the staging subject, audit-written; opt-in PG persistence of staged actions; agent routing moves confirmed execution to the caller's paired local agent) plus read-only `code_search` (ripgrep-or-walk, same sandbox), `process_list`, agent-only `screenshot` (headless Chrome on the paired machine; the server never fetches caller URLs), `list_dir` (capped directory listing, same sandbox), and git inspection `git_status`/`git_diff`/`git_log` (same sandbox; non-repo paths are error results); continuity tools `task_state_set`/`task_state_get`/`checkpoint_create`; memory tools `memory_save`/`memory_search`/`memory_list` (data-plane, no confirm gate — confidence 0.9, `mcp:<client>` provenance, ownership-predicated, kill-switch-gated saves, no MCP delete; documented in SECURITY.md §2.0b); project tools `project_create`/`project_list`. |
...
| Dashboard | Phase 5 (Jinja2 + HTMX; script vendored at `/static/htmx.min.js`, styles at `/static/app.css`): chat-first shell — `/dashboard` 303s to `/dashboard/chat`, single left sidebar (New chat, features, search + history, user footer), light-minimal default with dark toggle; `/dashboard/overview` keeps the old cards (owned count cards + 10 recent sessions), sessions index + per-session detail rendering the shared projection (`core/projection.py`, also backing the graph endpoint), cross-session task board, memory management (browse/filter/search, explicit create, audited owner-scoped delete — `INVINCIBLE_MEMORY=0` blocks creation only), memory-graph view (2026-09-06: `/dashboard/memory/graph` server-rendered SVG — center-radial project clusters, source-colored dots, timeline strip; Level 1 derived relationships via `core/memory_projection.py`, JSON sibling `GET /memories/graph` as the permanent contract for a future UI redesign), usage view with UTC day buckets (JSON sibling `GET /usage`, window clamped 1–90 days), settings page (system flags, read-only provider/routing panel, password forms), machines page (`/dashboard/machines`: per-machine online state, platform, auto-discovered capabilities, read-only), webchat (`/dashboard/chat`: sidebar history via `GET /dashboard/chat/list`, new-chat, BYOK model picker, live SSE streaming — same `core/chat_service.py` pipeline as `/v1/chat/completions`, so history is shared with API sessions; agent modes v2 — plan (read-only tools, plan only), manual (browser approvals per action, default), auto (acts without asking) — over the paired-PC tool path with `core/webchat_agent.py` running the loop). The whole surface resolves **session cookies only** (`require_user_session`) — foreign resources are byte-identical to unknown ones. |
| Harness | Machine-harness layer over the Phase 10 agent (`core/harness_*`, `docs/HARNESS_PLAN.md`): typed event bus (`harness_events`/`harness_bus`, workflow-scoped rows persisted to `workflow_events`), unified policy gate (`harness_policy`), dependency-injected loop spine + context hydration/compaction (`harness_runtime`), agent handoff router + parallel supervisor (`harness_router`/`harness_supervisor`, task-aware system prompts: shared base + read/do/plan overlays via `classify_task`/`build_system_prompt`, minimal per-step prompts via `build_subagent_prompt`), durable suspend/resume approvals (`harness_approvals`, migrations `0011`/`0012`); outbound-only WS relay (`WS /agent/ws`, WS-first with long-poll fallback, per-machine inventory with capabilities) plus read-only inspector stream (`WS /harness/events`). |
